...
test("artist add resolves a numeric metadata-provider response ID", async (t) => {
  ...
  const client = new LidarrClient();
  t.after(async () => {
    await dbOps.deleteLidarrArtistIdMap(artistMbid);
    client._httpAgent.destroy();
    client._httpsAgent.destroy();
    client._httpsInsecureAgent.destroy();
  });
  ...
  assert.equal(await dbOps.getLidarrArtistIdMap(artistMbid), "705@deezer");
...
test("artist add rejects a MusicBrainz ID and name mismatch", async (t) => {
  ...
  assert.equal(await dbOps.getLidarrArtistIdMap(artistMbid), null);
});
...
test("artist add accepts a MusicBrainz alias when resolving a provider ID", async (t) => {
  ...
  assert.equal(artist.id, 42);
  assert.equal(await dbOps.getLidarrArtistIdMap(artistMbid), providerArtistId);
});
...
test("artist add succeeds when a provider mapping conflict follows a successful POST", async (t) => {
  ...
  t.after(async () => {
    await dbOps.deleteLidarrArtistIdMap(artistMbid);
    await dbOps.deleteLidarrArtistIdMap(existingMbid);
    client._httpAgent.destroy();
    client._httpsAgent.destroy();
    client._httpsInsecureAgent.destroy();
  });

  await dbOps.setLidarrArtistIdMap(existingMbid, providerId);
  client.resolveArtistAddConfiguration = async () => ({
    resolved: {
      rootFolderPath: "/music",
      qualityProfileId: 1,
  ...
  assert.equal(artist.id, 42);
  assert.equal(await dbOps.getLidarrArtistMbid(providerId), existingMbid);
});
...
test("Lidarr artist provider mappings reject reverse-ID conflicts", async (t) => {
  ...
  t.after(async () => {
    await dbOps.deleteLidarrArtistIdMap(firstMbid);
    await dbOps.deleteLidarrArtistIdMap(secondMbid);
  });

  await dbOps.setLidarrArtistIdMap(firstMbid, providerId);
  await assert.rejects(
  ...
  assert.equal(await dbOps.getLidarrArtistMbid(providerId), firstMbid);
});
