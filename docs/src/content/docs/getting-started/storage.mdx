...
| Jellyfin | Server URL, API key, user ID, and an existing music library that contains Aurral's downloaded files | Mount the Aurral Downloads Folder in Jellyfin at the same container path, or add a **Jellyfin** path mapping. |

Download clients are slskd, SABnzbd or NZBGet, and deemix. Each writes completed files to its own folder; Aurral must be able to read that folder at the same path or through a path mapping.
...
`./config:/config` is only for Aurral's job queue, cached images, staging files, and diagnostic reports. Settings, users, and the library index live in Postgres. Logs go to stdout. Do not share `/config` with Lidarr, Navidrome, Plex, Jellyfin, or a download client.
...
| deemix | Download folder | `/data/downloads/deemix` |
...
| Jellyfin | Music library | An existing Jellyfin library that includes `/data/downloads/aurral`. Aurral does not create it. |
...
9. Open **Settings > Playback**, connect Navidrome, Plex, or Jellyfin, and follow the path rules below.
...
2. **File check:** the **Lidarr library** section of **Settings > Storage health** must be able to read the paths.
...
4. **Playback check:** Navidrome, Plex, or Jellyfin must have the Downloads Folder in its own filesystem and must scan it.
...
- **Applies to**: the application that reports the path, such as **Lidarr**, **slskd**, **NZBGet**, or **SABnzbd**.
...
For a native Windows service, the remote path can be `N:\ServerFolders\Music`. Mount that host folder in Aurral, then map the Windows path to the Aurral container path. Run **Settings > Storage health > Run checks** after saving the mapping.

### Navidrome sees a different path

Aurral publishes Navidrome playlists through the Subsonic API, so publishing does not use a path mapping. A **Navidrome** remote path mapping only helps Aurral recognize files that Navidrome playlists still use before automatic cleanup.
