...
Aurral keeps each local password encrypted with the instance's settings key, which is also stored in Postgres, so treat database backups as sensitive.
...
Aurral uses the Subsonic API to publish playlists, so publishing does not use a path mapping. A remote path mapping with **Applies to** set to **Navidrome** only translates the file paths in Navidrome playlists that Aurral checks before automatic cleanup. The automatically managed **Aurral Playlists** library must use the same absolute path in Aurral and Navidrome. Mount the shared download tree at matching paths in both applications.

Reused Lidarr tracks do not need matching path strings because Aurral resolves indexed songs by their exact recording MBID. Generated tracks use the exact indexed Navidrome file path or recording MBID first. If neither matches, Aurral uses an indexed song with the same title and artist (and album, when several match), but only when exactly one song matches. Navidrome still needs its own library for those files. Use a separate [Remote Path Mapping](/getting-started/storage/#paths-reported-by-lidarr-or-download-clients) only when Aurral cannot read a path reported by Lidarr or a download client.
...
If a new track is not indexed and no indexed song matches its recording MBID or its exact title and artist, Aurral leaves it out until Navidrome scans it. It never substitutes a same-title track from another artist. Navidrome scans the **Aurral Playlists** library, not only your main Lidarr library. If the library appears but has no tracks:
...
- run a Navidrome scan, or select **Refresh** in Aurral's Library, which asks Navidrome to scan afterwards (saving the Navidrome settings starts a scan only when a setting changed);
- wait for the scan to finish.

## Files used by Navidrome playlists

Before automatic cleanup deletes a file, Aurral reads every Navidrome playlist and keeps any file that a playlist still uses. The Navidrome account in **Settings > Playback > Navidrome** must be a Navidrome administrator so Aurral can see other users' private playlists. With a non-admin account, or while Navidrome is unreachable, Aurral keeps every file that automatic cleanup would delete. If Navidrome reports playlist paths that differ from Aurral's, add a remote path mapping with **Applies to** set to **Navidrome**.
