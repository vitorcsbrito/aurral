...
## Downloads fail with "Track matcher (bundled beets runtime) is unavailable"

Check the startup log for `beets matcher runtime self-test failed` and the `matcher` entry in `/api/health`. The Docker image ships the matcher. From a source checkout, create a virtual environment, run `pip install -r backend/matcher/requirements.txt`, and set `AURRAL_MATCHER_PYTHON` to that environment's `bin/python`.
