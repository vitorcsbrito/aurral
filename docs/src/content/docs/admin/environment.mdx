...
Installs created before the Postgres switch keep their data in `aurral.db` inside the data directory. Copy it into Postgres once, with Aurral stopped:

```bash
docker compose run --rm aurral node backend/scripts/migrateSqliteToPostgres.js
```

The script reads `aurral.db` from the data directory (override with `--sqlite <path>`), applies the Postgres schema, and inserts every row it finds, skipping rows that already exist. Add `--dry-run` to see row counts without writing, or `--truncate` to empty the Postgres tables first. `aurral.db` is left untouched; delete it once you have confirmed the migrated install.
