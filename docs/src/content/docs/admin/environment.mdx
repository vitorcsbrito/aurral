...
Genres, search text, and recency are rebuilt by the database as rows arrive, so they are not copied. The background job queue keeps its own `honker.db` file; leave it in place. `aurral.db` is left untouched; delete it once you have confirmed the migrated install.
