...
Upgrade jobs have a lower priority than missing-track downloads. Aurral uses slskd, Usenet, and deemix for upgrades, in the configured source-priority order. yt-dlp is not used for upgrades. It stops after the first source provides a valid, strictly better file. It waits for the next interval before it tries another automatic improvement for that track.
