...
Spotify returns playlist items in pages of up to 50. Aurral follows every page, so very large playlists can take longer to sync. If the items Aurral receives don't match the total Spotify reports, the sync fails without replacing the saved Aurral tracklist. Spotify only provides playlist item details for playlists you own or collaborate on.
...
| Artist ID (optional) | `artistMbid`, `artistId`                                |
| Album ID (optional)  | `albumMbid`, `releaseGroupMbid`, `albumId`              |
| Track ID (optional)  | `trackMbid`, `recordingMbid`, `recordingId`, `mbid`     |
...
Aurral can use one file in multiple playlists. This prevents a second download of the same file. Before it downloads a track, Aurral also looks in the track's expected `Artist/Album` folder under the Downloads Folder (under `_flows/<flow-id>` for a flow) and reuses an audio file named after the track.
...
Aurral also checks the playlists on each configured Plex, Navidrome, and Jellyfin server before it deletes a file automatically, for example when a flow refreshes, a synced import removes a track, or an upgrade replaces a file. A file that one of those playlists still uses, such as a flow track saved to a playlist in Feishin, stays at its original path, and Aurral tries the cleanup again before later library scans. If a server cannot be checked, Aurral keeps the file. Deleting a flow or playlist, removing a track from a playlist, or resetting playlists deletes files as before.
...
If Lidarr runs on Windows and Aurral runs in Docker, open **Settings > Storage health**, select **Run checks**, and read the **Lidarr library** section.

Use the reported path to add a mapping under **Settings > Download clients > Remote path mappings**.

Navidrome must scan the corresponding Lidarr library before Aurral can add a reused track to a Navidrome API playlist. See [Navidrome](/integrations/navidrome/#filesystem-paths).
