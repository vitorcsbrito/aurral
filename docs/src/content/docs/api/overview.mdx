...
as Last.fm, ListenBrainz, or Koito account linking, local play events, library
favorites, and creating, importing, or adopting playlists. Creating a flow also
needs a user session. Aurral returns `403` when an API key calls one of these
operations, except flow creation, which returns `400`.
...
- If an instance API key calls an operation that needs a user account, Aurral
  returns `403`. Creating a flow is the exception: it returns `400`.
